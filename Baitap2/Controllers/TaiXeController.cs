...
//    .FirstOrDefault();
//        }
//        if (c != null && c.TaiXeId != null && c.TaiXeId != taiXe.Id)
//        {
//            return View(null);
//        }
//        return View(c);
...
public class TaiXeController : Controller
{
    ...
    public IActionResult Index(int? id, int? ignoreId)
    {
        ...
        // 👉 truyền xuống view để so sánh SignalR
        ViewBag.TaiXeId = taiXe.Id;

        // ❌ nếu vừa từ chối → không load lại cuốc
        if (ignoreId != null)
            return View(null);
        ...
        else
        {
            c = _context.ChuyenDis
                .Where(x => x.TrangThai == TrangThai.DangTimTaiXe
                         && x.TaiXeId == null
                         && x.LoaiXe == taiXe.LoaiXe)
                .OrderByDescending(x => x.Id)
                .FirstOrDefault();
        }

        // 👉 nếu cuốc đã có tài xế khác → ẩn luôn
        if (c != null && c.TaiXeId != null && c.TaiXeId != taiXe.Id)
            return View(null);
